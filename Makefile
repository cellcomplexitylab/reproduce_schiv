...
bargraph-alive-cells-topics-no-bec-no-HIV.pdf simplex-topics-alive-cells-no-bec-no-HIV.pdf SAHA-signature-vs-HIV.pdf PMA-signature-vs-HIV.pdf: alivecells.tsv out-alive-no-bec-no-HIV.txt
	R -f scripts/plot-topics-alive-cells.R

# Print genes in signatures for pathway analysis.
PMA-weights.csv SAHA-weights.csv PMA-gene_ids.txt SAHA-gene_ids.txt: gene_annotations.tsv alivecells.tsv wfreq-alive-no-bec-no-HIV.txt
	R -f make-word-signatures-alive-cells.R
...
out-PMA.txt:
	python LDA-3-groups-PMA.py

# LDA without batch-effect correction.
out-PMA-no-bec.txt:
	python LDA-3-groups-PMA-no-bec.py

# LDA with batch-effect correction.
out-SAHA.txt:
	python LDA-3-groups-SAHA.py
