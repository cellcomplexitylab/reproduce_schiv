...
#class warmup_scheduler(torch.optim.lr_scheduler.SequentialLR):
...
def model(data=None):
   ...
   with pyro.plate("topics", K, dim=-1):
      ...
      one_over_K = torch.ones(1, device=X.device, dtype=X.dtype) / K
      topic_weights = pyro.sample(
            name = "alpha",
            fn = dist.Gamma(one_over_K, 1.)
      ).unsqueeze(-2)
      # Sample word frequencies by topic.
      one_over_G = torch.ones(G, device=X.device, dtype=X.dtype) / G
...
def guide(data=None):
   ...
   topic_weights_posterior = pyro.param(
         ...
         lambda: torch.ones(K, device=X.device, dtype=X.dtype),
         constraint = torch.distributions.constraints.positive
   )
   word_freqs_posterior = pyro.param(
         "word_freqs_posterior",
         lambda: torch.ones((K,G), device=X.device, dtype=X.dtype),
   ...
   doc_topics = pyro.param(
         "doc_topic_posterior",
         lambda: torch.ones((ndocs, K), device = X.device, dtype=X.dtype),
         constraint = torch.distributions.constraints.greater_than(0.5)
   )
   with pyro.plate("documents", ndocs, dim=-1):
      pyro.sample(
...
scheduler = pyro.optim.PyroLRScheduler(
      ...
      optim_args = {
         ...
         "optim_args": {"lr": 0.01}, "warmup": 100, "decay": 9500,
      },
